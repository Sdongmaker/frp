  ...
  <div class="visitor-edit-page">
    <div class="edit-header">
      <nav class="breadcrumb">
        <router-link to="/visitors" class="breadcrumb-item">访客</router-link>
        <span class="breadcrumb-separator">›</span>
        <span class="breadcrumb-current">{{ isEditing ? '编辑访客' : '新建访客' }}</span>
      </nav>
...
const formRules: FormRules = {
  name: [
    ...
    { min: 1, max: 50, message: '长度应在 1 到 50 之间', trigger: 'blur' },
  ],
  ...
  bindPort: [
    { required: true, message: '绑定端口不能为空', trigger: 'blur' },
    {
      validator: (_rule, value, callback) => {
        ...
        if (value > 65535) {
          callback(new Error('绑定端口不能超过 65535'))
          return
        }
...
const loadVisitor = async () => {
  ...
  try {
    ...
    ElMessage.error('加载访客失败：' + err.message)
...
const handleSave = async () => {
  ...
  try {
    ...
    if (isEditing.value) {
      ...
      ElMessage.success('访客已更新')
    } else {
      await visitorStore.createVisitor(data)
      ElMessage.success('访客已创建')
    }
