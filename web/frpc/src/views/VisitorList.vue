...
path = "./frpc_store.json"</pre>
      ...
      <template v-else>
        ...
        <div v-else class="empty-state">
          ...
          <p class="empty-hint">点击"新建访客"创建一个。</p>
        </div>
      </template>
    </div>

    <ConfirmDialog v-model="deleteDialog.visible" title="删除访客"
...
const handleDelete = (name: string) => {
  ...
  deleteDialog.message = `确定要删除访客 "${name}" 吗？此操作无法撤销。`
